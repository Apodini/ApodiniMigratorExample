...
public struct User: Codable {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        try container.encode(try [Event].from(participatingEvents, script: 8), forKey: .participatingEvents)
        try container.encode(password, forKey: .password)
        try container.encode(try [Review].from(reviews, script: 10), forKey: .reviews)
    ...
    public init(from decoder: Decoder) throws {
        ...
        participatingEvents = try [Event].from(try container.decode([Event].self, forKey: .participatingEvents), script: 9)
        password = try container.decode(String.self, forKey: .password)
        reviews = try [Review].from(try container.decode([Review].self, forKey: .reviews), script: 11)
